...
def einOutshape(formula, shape_A, shape_B):
    # 解析爱因斯坦求和公式
    input_A, input_B, output = re.split("->|,", formula)

    # 创建一个字典来存储每个索引对应的维度
    index_to_dim = {}

    # 将 A 的索引和维度映射到字典中
    for idx, dim in zip(input_A, shape_A):
        index_to_dim[idx] = dim

    # 将 B 的索引和维度映射到字典中
    for idx, dim in zip(input_B, shape_B):
        index_to_dim[idx] = dim
...
def Einsum2Matmul(equation, mgtensor, input1, **kwargs):
    ...
    if (
        ...
        if dtype_ == "complex32Toririhalf" or dtype_ == "complex32Torriihalf":
            ...
            output = (
                torch.view_as_real(mgtensor.nexttensor)
            ...
            torch.view_as_real(mgtensor.nexttensor).flatten()[: tmp.numel()].view(
                tmp.shape
            ).copy_(tmp)
        ...
        input1 = input1.permute(
            [ein_list[1].find(x) for x in ein1permuted]
        ).contiguous()
...
def torch_einsum(equation, input_0, input_1, **kwargs):
    ...
    dtype_ = kwargs.get("dtype_")
    ##########  modify equation and tensors ##########
    if input_0.dtype == torch.complex32:
        eq_org = equation
...
def parse_args():
    ...
    parser.add_argument(
        "--job_select", type=int, default=0, help="0: _nnTo1n, 1: _pairwiseTo1n"
    )
    parser.add_argument(
        "--warmup",
        type=int,
        default=1,
        help="whether to warm up for nccl, 0: False, 1: True",
    )
    parser.add_argument(
        "--data_type", type=int, default=0, help="0: complex32, 1: complex64"
    )
    parser.add_argument(
        "--is_scale",
        type=int,
        default=1,
        help="0: not scale, 1: scale, when complex32, it'd be 1 to guarantee precision",
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=0,
        help="0: truetask, 1: trainning algos and scales",
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default="complex32",
        help="optionals: int8, int8kernel, int4kernel, complex32",
    )
...
def compareWithBenchmark(
    cat_res, args, ntask, bitstrings=None, fakebenchmark=None, **kwargs
):
    ...
    if fakebenchmark is not None:
        ...
        fidelity = (
            (
                (groundTruth.conj() @ cat_res.reshape(-1)).abs()
                / (
                    groundTruth.abs().square().sum().sqrt()
                    * cat_res.abs().square().sum().sqrt()
                )
            )
            .square()
            .item()
        ...
        if world_rank == 0:
            print(
                f"Compared with complex64              : {round(fidelity, 8)}",
                flush=True,
            )
        return
    ...
    if args.tensorNetSize == "640G":
        ...
        if subtask_idx == 0:
            print(
                f"fidelity of task {ntask*subtasks-1} on rank{subtask_rank}: {round(fidelity, 6)}",
                flush=True,
            )
        ...
        if world_rank == 0:
            ...
            print(
                f"fidelity / expected          : {round(fidelity/expected, 5)}",
                flush=True,
            )
