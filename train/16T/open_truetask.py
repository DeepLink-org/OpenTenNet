...
cont_file = 'TensorNetwork/sc41_reproduce_scheme_n53_m20_ABCDCDAB_3000000_einsum_13_open.pt'
nsch = torch.load(f'TensorNetwork/open_sc41_nsch_split{split}_mg{mgmodes}_1646_splitmn.pt')
...
ans = scale_class.rescale(task_id, calc_task(task_id, **kwargs)[0].to(dtype=torch.complex64))
torch.cuda.synchronize()
time_end = time.time()
print(f"rank {world_rank} used time {round(time_end-time_begin, 3)}s", flush = True)
...
if world_rank == 0:
    ...
    print(f"ans on rank{subtask_rank} occupies {8.*ans.numel()/2**30} G")
