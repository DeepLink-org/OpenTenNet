...
from utils import torch_einsum as EinsumGeneral
import utils
import warnings
...
nsch = torch.load(
    f"TensorNetwork/640G/640G_rep_nsch640_split{split}_mg{mgmodes}_splitmn.pt"
)
            ...
            if step["type"] == 1:
                ...
                if flati > 1:
                    ...
                    tensors[i] = tensors[i].flatsplit(
                        flati, split
                    )  # convert MgTensor -> MgTensorSplit
                if flatj > 1:
...
for s in range(ntask):
    ...
    if s == 0:
        if typeCal == "complexHalf" and args.is_scale:
            ans = scale_class.rescale(
                task_id,
                calc_task(task_id, **kwargs)[0].to(dtype=torch.complex64),
                **kwargs,
            )
        else:
            ans = calc_task(task_id, **kwargs)[0]
...
if world_rank == 0:
    ...
    print(f"Truetask used time {round(time_end-time_begin, 3)}s", flush=True)
######################### Reduce answer ########################################
torch.cuda.synchronize()
time_begin = time.time()
